# R function converting Output_peak.txt or Output_cluster.txt from the
# function MS.clust of MSeasy into a file compatible with ARISTO webtool
# http://www.ionspectra.org/aristo/

# The format for Aristo batch search looks like >Name: 752225-5899 42 0.9856
# 57 395.2356 ..
MSeasyToARISTO <- function(filename = "", outfilename = "", cluster = "") {
    ...
    if ((filename != "") == FALSE || missing(filename)) {
        ...
        filename <- tk_choose.files(default = paste(getwd(), "/*.txt", sep = ""), 
            caption = "Please, select a txt file", multi = FALSE, filters = matrix(c("your txt file", 
                "*.txt"), ncol = 2))
    }
    ...
    close(zz)
    print(paste("A data file", outfile, ".txt has been generated in the folder:", 
        Mypath, cat("\n")))
