# R function converting Output_peak.txt or Output_cluster.txt obtained by the
# function MS.clust of MSeasy into a MSP file for NIST mass spectral

# The format MSP looks like Name: 752225-5899 DB#: 7598 Num Peaks: 77 42
# 0.9856; 57 395.2356; ....
MSeasyToMSP <- function(filename = "", outfilename = "", cluster = "", autosearch = FALSE) {
    ...
    if ((filename != "") == FALSE || missing(filename)) {
        require("tcltk")
        filename <- tk_choose.files(default = paste(getwd(), "/*.txt", sep = ""), 
            caption = "Please, select the text file", multi = FALSE, filters = matrix(c("your text file", 
                "*.txt"), ncol = 2))
    }
    ...
    for (j in 2:dim(d)[1]) {
        ...
        cat(" ", paste("Name: ", dname), paste("DB#: ", dDB), paste("Num Peaks: ", 
            dPeak), file = zz, sep = "\n")
        if (dPeak > 20) {
    ...
    close(zz)
    print(paste("A data file", outfile, ".MSP has been generated in the folder:", 
        Mypath, cat("\n")))
    if (autosearch == FALSE) {
