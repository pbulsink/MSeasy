# R function for connection between R and NIST mass spectral library search
# tool on Windows require NIST MS (Mass Spectral) Search Program Version 2.0
# or higher
SearchNIST <- function(mspfile = NULL) {
    ...
    if ((Sys.info()["sysname"]) != "Windows") {
        ...
        if (file.exists(file.path("C:", "Windows", "win.ini")) == TRUE) {
            ...
            nistpath <- readLines(file.path("C:", "Windows", "win.ini"))[pmatch("Path32", 
                readLines(file.path("C:", "Windows", "win.ini")))]
            if (is.na(nistpath) != TRUE) {
                nistpath <- file.path(unlist(strsplit(nistpath, split = "="))[2])
                
            }
            if (is.na(nistpath)) {
                nistpath <- readLines(file.path("C:", "Windows", "win.ini"))[pmatch("Path16", 
                  readLines(file.path("C:", "Windows", "win.ini")))]
                print("Your NIST MS program version is <2.0 or impossible to detect NIST")
                if (is.na(nistpath)) {
                  require(tcltk)
                  nistpath <- tclvalue(tkchooseDirectory(initialdir = getwd(), 
                    title = "Please, select the MS SEARCH directory, should be c:/nist/mssearch/"))
                  print(nistpath)
                }
            }
        ...
        if (file.exists(file.path(nistpath, "AUTOIMP.MSD")) == FALSE) {
            # create AUTIMP.MSD file secondlocatorpath<-paste(nistpath,'FILESPEC.FIL',
            # sep='')
        ...
        if (exists("mspfile") == FALSE) {
            require(tcltk)
            mspfile <- tk_choose.files(default = paste(getwd(), "/*.msp", sep = ""), 
                caption = "Please, select the MSP file", multi = FALSE, filters = matrix(c("your msp file", 
                  "*.msp"), ncol = 2))
            # zz<-file(paste(nistpath,'FILESPEC.FIL', sep=''), 'w')
            zz <- file("FILESPEC.FIL", "w")
            cat(paste(file.path(mspfile), "Overwrite", sep = " "), file = zz, 
                sep = "\n")
        ...
        code <- paste(file.path(nistpath, "nistms$.exe"), " /Instrument /par=2", 
            sep = "")
        ...
        print(paste("A file called ResultsFromNIST.txt has been generated in ", 
            Mypath, sep = " "))
        # a parser for that file is needed clean unused files from directories needed
