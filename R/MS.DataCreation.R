MS.DataCreation <- function(DataType = "CDF", path = "", pathCDF = "", mz, N_filt = 3, 
    apex = FALSE, quant = FALSE) {
    ...
    if (DataType == "Agilent" || DataType == "ASCII") 
        {
            ...
            if (DataType == "Agilent") 
                {
                  ...
                  Mypath <- paste("output_MSDataCreation", "_", "result", Date, 
                    sep = "")
                  dir.create(Mypath)
                  ...
                  files <- list.files(path = path1, pattern = filepattern, recursive = TRUE, 
                    full.names = TRUE)  #full.names conserve the full path to the file
                  list_data <- files
                  print(files)
                  ...
                  ### keep the firsts characters in the file names of the .d files Start change
                  ### specific for CDF files
                  ...
                  for (p in 1:length(test_pb1)) if (is.na(test_pb1[p]) == TRUE) {
                    problem1 <- 1
                    pb1 <- p
                  }
                  
                  for (k in 1:length(test_pb2)) if (is.na(test_pb2[k]) == TRUE) {
                    problem2 <- 1
                    pb2 <- k
                  }
                  ...
                  if (problem2 == 1) 
                    # start change cat('There is a problem in the export3ddata for ',
                  # an_data[pb2])
                  ...
                  for (m in 1:length(an)) {
                    ma_test <- as.matrix(readLines(paste(paste(path, "/", sep = ""), 
                      list_rteres[m], sep = ""), n = -1))
                    skip_value[m] <- as.numeric(agrep("scan scan scan", ma_test))
                  }
                  # search for differences in user defined mz vector and reel mz present in CDF
                  # files
                  ...
                  cdf_error <- function() {
                    ...
                    if (pbmzmax != 0) {
                      print(paste("mz maximum value was set to ", min(mz_max), 
                        cat("\n")))
                      pbmzmax <- 0
                    }
                  ...
                  errorrteres <- function() {
                    Rte <- read.table(paste(paste(path, "/", sep = ""), list_rteres[m], 
                      sep = ""), skip = skip_value[m] + 1, blank.lines.skip = TRUE, 
                      fill = TRUE)
                    ...
                    for (co in 1:nrow(Rte)) if (Rte[co, 11] == "") {
                      Rte[co, 11] <- Rte[co, 10]
                      Rte[co, 10] <- Rte[co, 9]
                      Rte[co, 9] <- Rte[co, 8]
                      Rte[co, 8] <- Rte[co, 7]
                    ...
                    Rte <- Rte[, c(1:5, 9, 11)]
                    colnames(Rte) <- c("peak", "RT", "first_scan", "max_scan", 
                      "last_scan", "quantification1", "quantification2")
                    ## Treat CDF files 3d data start change
                    obj <- xcms::xcmsRaw(files[m])
                    Td <- t(obj@env$profile)
                    colnames(Td) <- c(min(obj@mzrange):max(obj@mzrange))
                    
                    if (is.na(match(mz[length(mz)], colnames(Td))) == TRUE) {
                      Td <- Td[, match(mz[1], colnames(Td)):match(max(obj@mzrange), 
                        colnames(Td))]
                      
                    } else {
                      Td <- Td[, match(mz[1], colnames(Td)):match(mz[length(mz)], 
                        colnames(Td))]
                    }
                    ...
                    for (pic in 1:nrow(Rte)) if (as.numeric(as.vector(Rte$last_scan))[pic] > 
                      as.numeric(dim(Td)[1])) {
                      # number of the last scan in the file profile is a matrix with col=mz and row
                      # =scan number
                      a_env[av] <- pic
                      av <- av + 1
                    }
                    # end of change Treat export3d files removed
                    
                  }  #end test error
                  
                  for (m in 1:length(an)) {
                    result <- try(errorrteres(), silent = TRUE)
                    if (class(result) == "try-error") {
                      pblist[pb] <- list_rteres[m]
                      pb = pb + 1
                      assign("pblist", pblist, envir = as.environment("e1"))
                      next
                    }
                  }
                  # for (m in 1:length(an)){result<-try(errorrteres(), silent=TRUE);
                  # if(class(result) == 'try-error'){pblist[pb]<-list_rteres[m]; pb=pb+1;
                  # return(pblist);next;}}
                  ...
                  if (pblistl != 0) {
                    ...
                    eval(cat("A problem occured in the rteres.txt files listed below \n Check the pkty column for missing value for the first peak \n"), 
                      envir = as.environment("e1"))
                    pblist <- data.frame(pblist)
                    eval(print(pblist), envir = as.environment("e1"))
                    stop
                  } else {
                    for (m in 1:length(an)) {
                      ## rteres files treatment
                      ...
                      ## Remove incomplete peaks that are at the beginning or at the end of the
                      ## chromatogram, i.e. with missing scans in the Export3d
                      ...
                      for (pic in 1:nrow(Rte)) if (as.numeric(as.vector(Rte$first_scan))[pic] < 
                        1) {
                        # the first scan of the file IS 1 (I hope so)
                        a_env[av] <- pic
                        av <- av + 1
                      }
                      ...
                      ## Each part of the list contains a matrix where each row corresponds to a
                      ## given peak in the analysis
                      
                      if (quant == TRUE) {
                        # changes here tempo<-matrix(nrow=nrow(Rte), ncol=ncol(Td)+2)
                        tempo <- matrix(nrow = nrow(Rte), ncol = length(mz) + 
                          3)
                        colnames(tempo) <- c("RT", "quantification1", "quantification2", 
                          mz)
                        tempo[, 1] <- as.numeric(as.vector(Rte$RT))
                        tempo[, 2] <- as.numeric(as.vector(Rte$quantification1))
                        tempo[, 3] <- as.numeric(as.vector(substr(Rte$quantification2, 
                          1, 4)))
                        ...
                        for (Pi in 1:nrow(Rte)) {
                          ...
                          if (apex == TRUE) {
                            ...
                            # start change a <- as.matrix(Td[Td$scan_number == sc, 2:ncol(Td)])
                            a <- as.matrix(Td[sc, ])
                          ...
                          ## If apex=FALSE, look for scanmin and scanmax for each peak and select 5% scan
                          ## around scan_max
                          
                          if (apex == FALSE) {
                            Rang <- as.numeric(as.vector(Rte$last_scan))[Pi] - 
                              as.numeric(as.vector(Rte$first_scan))[Pi]
                            sc5 <- Rang * 5/100
                            sc <- round(as.numeric(as.vector(Rte$max_scan))[Pi] - 
                              sc5, digits = 0):round(as.numeric(as.vector(Rte$max_scan))[Pi] + 
                              sc5, digits = 0)
                            ...
                            if (length(sc) > 1) {
                              ...
                              tempo[Pi, 4:ncol(tempo)] <- apply(a, MARGIN = 2, 
                                FUN = mean, na.rm = TRUE)
                            } else {
                              sc <- as.numeric(as.vector(Rte$max_scan))[Pi]
                        ...
                        for (Pi in 1:nrow(Rte)) {
                          ...
                          if (apex == FALSE) {
                            ...
                            if (length(sc) > 1) {
                              ...
                              tempo[Pi, 2:ncol(tempo)] <- apply(a, MARGIN = 2, 
                                FUN = mean, na.rm = TRUE)
                            } else {
                              sc <- as.numeric(as.vector(Rte$max_scan))[Pi]
                              # a<-as.matrix(Td[Td$scan_number==sc,2:ncol(Td)])
                    ...
                    if (quant == TRUE) {
                      colnames(data_fin) <- c("analysis", "retention_time", "quantification1", 
                        "quantification2", mz)
                      rownames(data_fin) <- 1:nrow(data_fin)
                      write.table(data_fin, file = paste(Mypath, "/", "initial_DATA", 
                    ...
                    # Rprof(NULL) summaryRprof(filename = 'Rprof.out')$sampling.time
                    print(paste("A data file has been generated in the folder:", 
                      Mypath, cat("\n")))
                    return(data_fin)
                  }  #end else
            ...
            
            {
                ...
                for (a in 1:length(L_an)) {
                  ...
                  # change to file.path instead of paste(path,'/',L_an[a], sep='') add
                  # check.names=FALSE for mz values
                  ...
                  if (missing(mz) == TRUE) {
                    ...
                    mz <- mzall
                    print(paste("mz = ", colnames(te)[3], ":", colnames(te)[dim(te)[2]], 
                      sep = ""))
                  }
                  ...
                  if (N_filt > 3) {
                    ...
                    for (i in N_filt:(dim(b)[1] - N_filt)) {
                      if (b[i, 2] > 5000 & (b[i, 2] > b[(i - 1), 2]) & (b[i, 2] > 
                        b[(i + 1), 2]) & (b[(i - 1), 2] > b[(i - 2), 2]) & (b[(i + 
                        1), 2] > b[(i + 2), 2]) & (b[(i - 2), 2] > b[(i - 3), 
                        2]) & (b[(i + 2), 2] > b[(i + 3), 2])) {
                        result[i] = b[i, 1]
                      }
                    }
                  ...
                  save(res_data_temp, file = paste(Mypath, "/", "save_list_temp.rda", 
                    sep = ""))
                }
                ...
                data_fin <- cbind(data_fin[, 1:2], data_fin[, which(colnames(data_fin) == 
                  mz[1]):which(colnames(data_fin) == mz[length(mz)])])
                write.table(data_fin, file = paste(Mypath, "/", "initial_DATA", 
                  ".txt", sep = ""), row.names = FALSE)
                # Rprof(NULL) summaryRprof(filename = 'Rprof.out')$sampling.time
                print(paste("A data file has been generated in the folder:", Mypath, 
                  cat("\n")))
                return(data_fin)
            }
    ...
    if (DataType == "CDF") 
        {
            ...
            if (is.null(pathCDF) == TRUE || missing(pathCDF) == TRUE) {
                cdffiles <- list.files(path = tclvalue(tkchooseDirectory(title = "Select a directory with CDF (mzXML) files AND peaklist files")), 
                  pattern = filepattern, recursive = TRUE, full.names = TRUE)
                if (length(cdffiles) == 0) {
                  cat("Stop No CDF or mzXML files detected \n ")
                }
                
                peakfiles <- list.files(path = dirname(cdffiles), pattern = "peaklist.txt", 
                  recursive = TRUE, full.names = TRUE)
                # if Agilent rteres.txt files are used instead of peaklist.txt
                if (length(peakfiles) == 0 && length(cdffiles) > 0) {
                  peakfiles2 <- list.files(path = dirname(cdffiles), pattern = "rteres.txt", 
                    recursive = TRUE, full.names = TRUE)
                  ...
                  if (length(cdffiles) == length(peakfiles)) 
                    {
                      ...
                      for (fi in 1:length(cdffiles)) {
                        peaklist <- read.table(file.path(dirname(cdffiles[fi]), 
                          "peaklist.txt"), header = TRUE, sep = "\t")
                        ...
                        if (length(grep("[Qq][Uu][Aa][Nn]", colnames(peaklist))) > 
                          0) {
                          quant = TRUE
                        }
                        ...
                        for (j in 1:peakslg) {
                          ...
                          # changer ici si un rtmin rtmax est donné et utiliser scantime calculer RT
                          # avec scantime vs scanindex
                          if (apex == FALSE) {
                            ...
                            sc5 = Rang * 5/100
                            MS = round(scantop[j] - sc5, digits = 0):round(scantop[j] + 
                              sc5, digits = 0)
                          }
                          if (apex == TRUE) {
                            scantop[j] = peaklist[j, "maxscan"]
                          ...
                          if (quant == TRUE) {
                            ...
                            if (length(grep("[Qq][Uu][Aa][Nn]", colnames(peaklist))) > 
                              ...
                              quantif[1, 1] <- paste(peaklist[j, grep("[Qq][Uu][Aa][Nn]", 
                                colnames(peaklist))[1]], sep = "\t")
                              quantif[1, 2] <- paste(peaklist[j, grep("[Qq][Uu][Aa][Nn]", 
                                colnames(peaklist))[2]], sep = "\t")
                            } else {
                              quantif[1, 1] <- paste(peaklist[j, grep("[Qq][Uu][Aa][Nn]", 
                                colnames(peaklist))[1]], sep = "\t")
                              quantif[1, 2] <- paste(peaklist[j, grep("[Qq][Uu][Aa][Nn]", 
                          ...
                          for (i in 1:length(MS)) {
                            Ma <- match(round(obj@env$mz[(obj@scanindex[scan] + 
                              1):obj@scanindex[scan + 1]], 0), colnames(an))
                            for (p in 1:length(Ma)) {
                              if (is.na(Ma[p]) == FALSE) {
                                an[i, Ma[p]] <- obj@env$intensity[(obj@scanindex[scan] + 
                                  1):obj@scanindex[scan + 1]][p]  #put the right intensity to the m/z
                              }
                              
                            }
                            scan = scan + 1
                          }
                          # faire moyenne des intensité/mz puis normalisation sur le max normalisation
                          # of m/z intensity first average intensity for each m/z and search of the m/z
                          ...
                          # en fonction des colnames de peaklist.txt ajouter RT et si quant=T aire
                          # airemax %aire RTmin RTsec ajouter les bons noms de colonnes
                          
                          if (quant == FALSE) {
                            peakstemp <- cbind(basename(cdffiles[fi]), RT[j], 
                              peakstemp)
                          }
                          if (quant == TRUE) {
                      ...
                      if (quant == TRUE) {
                        ...
                        if (length(grep("[Qq][Uu][Aa][Nn]", colnames(peaklist))) < 
                          2) {
                          print(paste(length(grep("[Qq][Uu][Aa][Nn]", colnames(peaklist))), 
                            " One Quantification columns have been found", sep = " "))
                          print(colnames(peaklist)[grep("[Qq][Uu][Aa][Nn]", colnames(peaklist))])
                          colnames(peaks)[3] <- colnames(peaklist)[grep("[Qq][Uu][Aa][Nn]", 
                            colnames(peaklist))]
                          colnames(peaks)[4] <- colnames(peaklist)[grep("[Qq][Uu][Aa][Nn]", 
                            colnames(peaklist))]
                        } else {
                          if (length(grep("[Qq][Uu][Aa][Nn]", colnames(peaklist))) > 
                            2) {
                            print(paste(length(grep("[Qq][Uu][Aa][Nn]", colnames(peaklist))), 
                              " Quantification columns have been found, Max is 2 /n Only the 2 first will be used", 
                              sep = " "))
                            print(paste(colnames(peaklist)[grep("[Qq][Uu][Aa][Nn]", 
                              colnames(peaklist))[1]], colnames(peaklist)[grep("[Qq][Uu][Aa][Nn]", 
                              colnames(peaklist))[2]], sep = " "))
                          }
                          colnames(peaks)[3] <- colnames(peaklist)[grep("[Qq][Uu][Aa][Nn]", 
                            colnames(peaklist))[1]]
                      ...
                      write.table(peaks, file = file.path(Mypath, "initial_DATA.txt"), 
                        quote = c(1, 2), row.names = FALSE)
                      print(paste("A data file has been generated in the folder:", 
                        Mypath, cat("\n")))
