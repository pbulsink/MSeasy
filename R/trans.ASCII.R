trans.ASCII <- function(path, mz) {
    ...
    if (flagmz == FALSE) {
        ...
        for (N_analyses in 1:length(L_analyses)) {
            # change file.path instead of paste(path..  an_tempo <- scan(paste(paste(path,
            # '/', sep = ''), L_analyses[N_analyses], sep = ''), blank.lines.skip = FALSE)
                  ...
                  if (is.na(an_tempo[i + 1]) == FALSE) 
                    if (is.na(an_tempo[i + 3]) == TRUE) 
                      if (is.na(an_tempo[i + 6]) == FALSE) {
                        chrom <- rbind(chrom, c(an_tempo[i + 1], an_tempo[i + 
                          2]))
                        k <- k + 1
                        v[k] <- i + 1
                      }
            ...
            for (nu in 1:(length(v) - 1)) {
                MS[[nu]] <- c(an_tempo[v[nu] + 3], an_tempo[v[nu] + 4])
            ...
            for (i in 1:length(MS)) {
                if (is.matrix(MS[[i]]) == TRUE) {
                  ...
                  for (p in 1:length(Ma)) if (is.na(Ma[p]) == FALSE) 
                    an[i, p] <- MS[[i]][Ma[p], 2] else (an[i, p] <- 0)
                } else {
            ...
            write.table(an, file = file.path(PathDate, L_analyses[N_analyses]), 
                row.names = FALSE)
        }
